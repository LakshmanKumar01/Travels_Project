...
public class JourneyService {
	...
	public void reScheduleJourney() {
		...
		if (orderToReschedule != null) {
			...
			List<Route> availableRoutes = getRoutes(orderToReschedule.getRoute().getSource(),
					...
					orderToReschedule.getRequestJourneyPlan().getNumberOfPassengers());

			if (!availableRoutes.isEmpty()) {
				// Update the journey date
				orderToReschedule.getRequestJourneyPlan().setJourneyDate(newDate);
	...
	private Order createOrder(LocalDate date, int passengers, Route selectedRoute) {
		...
		double bookingCost = selectedRoute.getTicketCost() * passengers;
		...
		newOrder.setRequestJourneyPlan(new Journey(date, passengers)); // Assuming Journey constructor takes date and
																		// passengers
